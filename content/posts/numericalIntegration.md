...
- **台形公式** の誤差は、分割数 $n$ を増やすとほぼ**直線的に減少** しており、  
  理論どおりおおよそ $(1/n^2)$ の**多項式的な収束** になっています。
- **DE公式** の誤差は、ある程度 $n$ を増やすと一気に**機械精度レベル(倍精度浮動小数点数の有効桁数は約15~16桁程度)** まで落ちて、それ以上はほとんど変化していません。これは誤差が$\exp(-c n)$ のような**指数的な速さで減衰** していることを反映しています。

このように、滑らかな関数 $e^x$ に対しても、
DE公式は台形公式と比べて、はるかに少ない分点数で高精度な結果を得られることがわかります。
