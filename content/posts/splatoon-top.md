...
public unsafe class TOP_Cross_Oversampled_Wave_Cannon : SplatoonScript
{
    ...
    private Config Conf => Controller.GetConfig<Config>();
    private PriorityData PriorityData => Conf.PriorityData ??= new();
    ...
    private List<string> GetMonitorSlots(bool isSupport)
    {
        ...
        var players = PriorityData.GetPlayers(x => x.IGameObject is IPlayerCharacter)?.ToList() ?? [];
...
internal class TOP_P6_Limiter_Cut_Wave_Cannon : SplatoonScript
{
    ...
    {
        var distanceFromCenter = MathF.Sqrt(MathF.Pow(npc.Position.X - CenterX, 2) + MathF.Pow(npc.Position.Z - CenterZ, 2));
        return npc.DataId == WaveCannonNpcDataId || MathF.Abs(distanceFromCenter - OuterRadius) <= 3f;
    }
    ...
    private static Vector3 PositionFromIndex(float index, float distance)
    {
        ...
        return new Vector3(
            CenterX + MathF.Sin((float)radians) * distance,
            0f,
            CenterZ - MathF.Cos((float)radians) * distance);
    }
...
internal unsafe class TOP_P6_Magic_Number_LB : SplatoonScript
{
    ...
    private void DrawRoleCombo()
    {
        ...
        {
            foreach(var role in Enum.GetValues<LbRole>())
            {
    ...
    private List<Dalamud.Game.ClientState.Objects.SubKinds.IPlayerCharacter> GetParty()
    {
    ...
    private static bool HasMagicNumberStatus(Dalamud.Game.ClientState.Objects.SubKinds.IPlayerCharacter player)
    {
