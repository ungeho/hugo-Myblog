...
を同じ器に押し込もうとした瞬間（この例では $(x - a)$ の計算）に、  
**器からあふれてしまったほう（小さい $x$ のほう）の情報が落ちる**、という現象が起きています。
...
情報落ちや桁落ちは**演算前の状態**に問題があり、式変形などで回避可能な場合があります。
