...
- $E$ のビット列がすべて 1 のとき（単精度なら $E = 255$、倍精度なら $E = 2047$）
  - 仮数部のビット列もすべて 0、つまり $m = 0$ のとき：  
    $\pm \infty$  
...
- **2 進数で小数を表現していること**  
    ...
    これを**循環小数**と呼び、有名な例として **0.1** が挙げられます。
...
$$
0.123 \rightarrow 0.12 \text{（小数第2位まで保持するフォーマットの場合、通常は切り捨て）}
$$
