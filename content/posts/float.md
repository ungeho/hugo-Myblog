...
多くのケースで$(x - a) + a \neq x$となっています。  
これは、**浮動小数点数が保持できる精度に限界があり、小さなxが大きなaとの差分計算で消えてしまうため**に起こる典型例です。  
よくある誤解として、桁落ちを「計算がおかしい」「演算が間違っている」と捉えてしまうことがあります。  
しかしこれは、**計算自体は仕様どおりに正しく行われており、その結果として情報が失われている** 状態です。  
> ここで起きている現象は丸め誤差ではありません。
> 演算は正確に行われていますが、浮動小数点数の精度では小さなxを保持できず
> **表現の限界**によってxの情報が消えてしまう**桁落ち**が原因です。
